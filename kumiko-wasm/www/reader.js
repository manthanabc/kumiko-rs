class Reader {
	...
	zoomOn (panel)
	{
	  ...
	  panel[0].style.background="transparent"
	...
	{
    if (i == 0) {
        let t = document.getElementsByClassName('panel');
        for(let i=0; i< t.length; i++) {
            t[i].style.background="white"
        }
    }
		if (i < 0) {
			this.currpanel = 'last';
	...
	drawPanels (imginfo)
	{
		...
		var i = 1;
		for (var p_index in imginfo['panels'])
		{
			var unknown = !this.known_panels.includes(parseInt(p_index));
			var p = imginfo['panels'][p_index];
			var [x,y,w,h] = [p.x, p.y, p.width, p.height];
